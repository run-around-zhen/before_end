...
struct segtree{
    ...
    void pushdown(int l, int r, int rt){
        ...
        if(laze[rt]){
            laze[rt * 2] += laze[rt];
            laze[rt * 2 + 1] += laze[rt];
            num[rt * 2] += 1U * (mid - l + 1) * laze[rt];
            num[rt * 2 + 1] += 1U * (r - mid) * laze[rt];
            laze[rt] = 0;
        }
        return ;
    }
    ...
    void updata(int l, int r, int ql, int qr, U val, int rt){
        if(l >= ql && r <= qr){
            laze[rt] += val;
            num[rt] += (r - l + 1) * val;
            return ;
    ...
    U qry(int l, int r, int ql, int qr, int rt){
        if(l >= ql && r <= qr){
            return num[rt];
        }
        pushdown(l, r, rt);
...
int main(){
    ...
    while(M --){
        ...
        if(op == 1){
            ...
            cin >> x;
            cout << T.qry(1, N, hc.id[x], hc.id[x] + hc.sz[x] - 1, 1) % mod<< endl;
        }
    }
    return 0;
}
