...
typedef vector<int> vi;const int M = 2e6 + 10;
int a[M], b[M], c[M], d[M], e[M], f[M], res[M], cnt[M], A[M], inv[M];
vi T[M];
...
ll get(int a, int b){
	if(a < b) return 0;
	return (ll)A[a] * inv[b] % P * inv[a - b] % P;
}
int main(){
	...
	while(k -- ){
		...
		//cout << sum1 << " " << sum2 << endl; 
		rep(i, 0, sum1 + 1){
			a[i] = (ll)get(sum1, i) * qpow(2, i) % P;
			//cout << a[i] << " " << endl;
		}
		rep(i, 0, sum2 * 2 + 1){
			b[i] = get(sum2 * 2, i); 
		}
		ntt.doit(a, b, sum1 + 1, sum2 * 2 + 1);
		rep(i, 0, ntt.N){
			res[(i + x + 1) * 2] = (res[(i + x + 1) * 2] + a[i]) % P;
		}
	}
	int Q;
	cin >> Q;
