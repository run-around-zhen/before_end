...
const int M = 2e6 + 10;
int a[M], b[M], c[M], d[M], e[M], f[M], A[M], B[M], ans[M];
vi T[M];
struct NTT{
...
int main(){
	...
	rep(i, 0, n) a[i] = A[i] * A[i] * A[i];
	rep(i, 0, m) b[i] = B[i];
	ntt.doit(a, b, n, m);
	rep(i, 0, ntt.N) ans[i] += a[i];
	rep(i, 0, n) a[i] = A[i] * A[i];
	rep(i, 0, m) b[i] = B[i] * B[i];
	ntt.doit(a, b, n, m);
	rep(i, 0, ntt.N) ans[i] -= 2 * a[i];
	rep(i, 0, n) a[i] = A[i];
	rep(i, 0, m) b[i] = B[i] * B[i] * B[i];
	ntt.doit(a, b, n, m);
	rep(i, 0, ntt.N) ans[i] += a[i];
	rep(i, n - 1, m) if(ans[i] % ntt.P == 0) res.pb(i);
	cout << res.size() << endl;
	for(auto i : res){
		cout << i - n + 2 << " ";
	} 
