...
struct NTT{
    ...
    void Merge(int m){
        ...
        while(H.size() >= 2){
            ...
            rep(i, 0, (int)T[x].size()) a[i] = T[x][i];
            rep(i, 0, (int)T[y].size()) b[i] = T[y][i];
...
int main(){
	...
	while(_ --){
		...
		ntt.Merge(n);
		int id = ntt.H.top().se;
		rep(i, 0, n + 1) A[i] = T[id][i];
		ntt.get_ln(A, B, n + 1);
		rep(i, 0, n + 1) B[i] = (ll)i * B[i] % mod;
		rep(i, 0, n + 1) B[i] = (- B[i] + mod) % mod;
		B[0] = n % mod;
		int res = 0;
		rep(i, 1, n + 1) res ^= B[i];
		cout << res << endl;
