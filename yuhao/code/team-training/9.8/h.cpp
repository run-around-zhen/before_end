...
const int N = 2e5 + 5;
int g[N + 10], dp[N + 10];
int mu[N + 10], flag[N + 10], p[N + 10], d_x[N + 10], d_kx[N + 10],f[N + 10], h[N + 10],sum_mu[N + 10], sum_d_kx[N + 10], sum_d_x[N + 10], sum[N + 10], sum_f[N + 10];
const int mod = 1e9 + 7;
int hh[N + 10];
int T, k, x;
void get_mu(){
...
void init(){
	get_mu();
	//rep(i, 1, N + 1) sum_mu[i] = (sum_mu[i - 1] + mu[i]) % mod;
	rep(i, 1, N + 1) d_x[i] = qpow(i, x);
	rep(i, 1, N + 1) sum_d_x[i] = (sum_d_x[i - 1] + d_x[i] * mu[i] + mod) % mod;
	rep(i, 1, N + 1) f[i] = abs(mu[i]);
	//rep(i, 1, N + 1) sum_f[i] = (sum_f[i - 1] + f[i]) % mod;
	//rep(i, 1, x + 1) sum[i] = (sum[i - 1] + qpow(i, k)) % mod;
...
int get_g(int n){
	...
	/*rep(i, 1, n + 1){
		ans = (ans + qpow(i, x) * mu[i] * get_h(n /)) % mod;
	}*/
	return g[n] = ans;
}
int get_dp(int n){
